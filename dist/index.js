...
__export(sqlite_schema_exports, {
  ...
  campaignInvitationsRelations: () => campaignInvitationsRelations,
  campaigns: () => campaigns,
  campaignsRelations: () => campaignsRelations,
  creators: () => creators,
  creatorsRelations: () => creatorsRelations,
  insertActivityLogSchema: () => insertActivityLogSchema,
  insertBrowserSessionSchema: () => insertBrowserSessionSchema,
  insertCampaignInvitationSchema: () => insertCampaignInvitationSchema,
...
var init_sqlite_schema = __esm({
  "shared/sqlite-schema.ts"() {
    ...
    users = sqliteTable("users", {
      ...
      tiktokSession: text("tiktok_session"),
    ...
    campaignInvitations = sqliteTable("campaign_invitations", {
      id: integer("id").primaryKey({ autoIncrement: true }),
    ...
    browserSessions = sqliteTable("browser_sessions", {
      id: integer("id").primaryKey({ autoIncrement: true }),
      userId: integer("user_id").references(() => users.id).notNull(),
...
var init_db = __esm({
  "server/db.ts"() {
    "use strict";
    init_sqlite_schema();
    dbPath = process.env.NODE_ENV === "production" ? "./prod.db" : "./dev.db";
...
var TIKTOK_APP_ID, TIKTOK_APP_SECRET, TIKTOK_REDIRECT_URI, TikTokAPI, tiktokAPI;
var init_tiktok_api = __esm({
  "server/api/tiktok-api.ts"() {
    "use strict";
    TIKTOK_APP_ID = process.env.TIKTOK_APP_ID || "7512649815700963329";
    TIKTOK_APP_SECRET = process.env.TIKTOK_APP_SECRET || "e448a875d92832486230db13be28db0444035303";
    TIKTOK_REDIRECT_URI = process.env.REPLIT_DOMAINS ? `https://${process.env.REPLIT_DOMAINS.split(",")[0]}/api/auth/tiktok/callback` : "https://11b0ef4e-37b9-4e43-b34e-8ecadadb92e9-00-39k23re3ie3qw.spock.replit.dev/api/auth/tiktok/callback";
    TikTokAPI = class {
    ...
    TikTokWebhookHandler = class {
      ...
      verifySignature(payload, signature, timestamp) {
        try {
          ...
          const expectedSignature = crypto.createHmac("sha256", this.WEBHOOK_SECRET).update(timestamp + payloadString).digest("hex");
...
var AIService = class {
  ...
  parseCreatorResponse(response) {
    try {
      const creators2 = JSON.parse(response);
      return Array.isArray(creators2) ? creators2 : [];
    } catch (error) {
...
var AutomationTester = class {
  ...
  async testCreatorOperations() {
    const creators2 = await storage.searchCreators("test");
    ...
    return {
      searchTest: "passed",
      searchResults: creators2.length,
...
async function registerRoutes(app2) {
  app2.get("/api/auth/tiktok", asyncHandler(async (req, res) => {
    const { tiktokAPI: tiktokAPI2 } = await Promise.resolve().then(() => (init_tiktok_api(), tiktok_api_exports));
    const authURL = tiktokAPI2.getAuthorizationURL();
    res.json({ authURL });
  }));
  app2.get("/api/auth/tiktok/callback", asyncHandler(async (req, res) => {
    ...
    try {
      const { tiktokAPI: tiktokAPI2 } = await Promise.resolve().then(() => (init_tiktok_api(), tiktok_api_exports));
      const tokenData = await tiktokAPI2.exchangeCodeForToken(code);
      await storage.storeTikTokTokens(1, {
        accessToken: tokenData.access_token,
        refreshToken: "",
        // TikTok doesn't provide refresh tokens in some flows
        expiresAt: new Date(Date.now() + tokenData.expires_in * 1e3),
        refreshExpiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3)
      });
      tiktokAPI2.setAccessToken(tokenData.access_token);
      res.json({
        success: true,
      ...
      logger.error("TikTok OAuth callback error", "auth", void 0, { error: error.message });
      throw new AppError("Failed to connect TikTok account", 500);
    }
  }));
  app2.get("/api/tiktok/status", asyncHandler(async (req, res) => {
    const { tiktokAPI: tiktokAPI2 } = await Promise.resolve().then(() => (init_tiktok_api(), tiktok_api_exports));
    ...
    if (envToken && !hasStoredToken) {
      tiktokAPI2.setAccessToken(envToken);
    } else if (hasStoredToken) {
      tiktokAPI2.setAccessToken(tokens.accessToken);
    }
    let isValid = false;
    let errorMessage = null;
    let scopeIssue = false;
    try {
      isValid = await tiktokAPI2.validateToken();
    } catch (error) {
    ...
    res.json({
      ...
      authURL: !isValid ? tiktokAPI2.getAuthorizationURL() : null,
  ...
  app2.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const creators2 = await storage.searchCreators(q);
      res.json(creators2);
    })
  );
  app2.post(
  ...
  app2.get("/api/campaigns/:userId", async (req, res) => {
    try {
      ...
      const campaigns2 = await storage.getCampaigns(userId);
      res.json(campaigns2);
    } catch (error) {
...
var PuppeteerAutomation = class {
      ...
      if (process.env.NODE_ENV === "production") {
        ...
        for (const path5 of possiblePaths) {
          try {
            ...
            if (existsSync(path5)) {
              launchOptions.executablePath = path5;
              console.log(`Using Chrome at: ${path5}`, "puppeteer");
              break;
            }
          } catch (error) {
            continue;
  ...
  async detectLoginStatus() {
    ...
    try {
      ...
      const authCookies = cookies.filter(
        (cookie) => cookie.name.includes("sessionid") || cookie.name.includes("sessionid_") || cookie.name.includes("sessionid_") || cookie.name.includes("sid_tt") || cookie.name.includes("passport_auth_token")
      );
  ...
  async discoverCreators(category, minFollowers = 1e4) {
    try {
      ...
      const creators2 = await this.page.evaluate(() => {
      ...
      return creators2.filter((creator) => creator.username);
    } catch (error) {
  ...
  parseFollowerCount(text2) {
    if (!text2) return null;
    const match = text2.match(/([\d.]+)([KMB]?)/i);
...
var envConfig = {
  ...
  optional: {
    ...
    "API_KEY": process.env.NODE_ENV === "production" ? void 0 : "dev-api-key",
    "GEMINI_API_KEY": "",
    "PERPLEXITY_API_KEY": "",
    "TIKTOK_APP_ID": process.env.NODE_ENV === "production" ? void 0 : "dev-app-id",
    "TIKTOK_APP_SECRET": process.env.NODE_ENV === "production" ? void 0 : "dev-app-secret"
  },
  sensitive: [
    "GEMINI_API_KEY",
    "PERPLEXITY_API_KEY",
